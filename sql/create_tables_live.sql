-- ============================================================================
-- OPDI Table Creation Script - LIVE Environment
-- Database: project_opdi
-- Warehouse: abfs://storage-fs@cdpdllive.dfs.core.windows.net/data/project/opdi.db/unmanaged
--
...
CREATE TABLE IF NOT EXISTS `project_opdi`.`osn_statevectors_v2` (
...
PARTITIONED BY SPEC (icao_aircraft_class)
STORED BY ICEBERG
TBLPROPERTIES (
...
CREATE TABLE IF NOT EXISTS `project_opdi`.`oa_airports` (
...
CREATE TABLE IF NOT EXISTS `project_opdi`.`oa_runways` (
...
CREATE TABLE IF NOT EXISTS `project_opdi`.`oa_navaids` (
...
CREATE TABLE IF NOT EXISTS `project_opdi`.`oa_airport_frequencies` (
    id INT, airport_ref INT, airport_ident STRING,
...
CREATE TABLE IF NOT EXISTS `project_opdi`.`oa_countries` (
...
CREATE TABLE IF NOT EXISTS `project_opdi`.`oa_regions` (
...
COMMENT '`project_opdi`.`osn_statevectors_v2` with added track_ids (generated based on callsign, icao24 grouping with 30 min signal gap intolerance) and H3 tags.'
PARTITIONED BY SPEC (DAY(event_time))
STORED BY ICEBERG
TBLPROPERTIES (
...
-- 2b. Flight List
CREATE TABLE IF NOT EXISTS `project_opdi`.`opdi_flight_list_v2` (
...
PARTITIONED BY SPEC (DAY(FIRST_SEEN))
STORED BY ICEBERG
TBLPROPERTIES (
...
COMMENT '`project_opdi`.`opdi_flight_events` table containing various OSN trajectory flight events.'

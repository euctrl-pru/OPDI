...
| 29 | **`ATOT`/`ALDT` are retained, not retired — decision 25 is corrected by its own rule.** | Decision 25 said a type is retired only where reference data shows its replacement better, then retired `take-off`/`ATOT` and the old `landing`/`ALDT` on the expectation that it would. Measured on 2026-06-05..07 it does not: `airborne` reaches 76.58% against `ATOT`'s 89.78%, and `touchdown` 80.37% against `ALDT`'s 95.92%. The A-CDM pair wins on timing — `airborne` +21.9 s against `ATOT` +25.0 s, and within a few seconds of truth at the three aerodromes whose reference resolves to the second — but timing was never the criterion decision 25 set. `events_v0.2.0` therefore publishes both families, and the paper argues the retention from the data rather than from the plan. The rule was right; applying it before the measurement existed was not. |
| 30 | **`airport_admit_on_ground` was on for two runs and never took effect, and nothing could see it.** | `process_month` projected the track table down to an explicit column list that omitted `on_ground`, and the gate consuming it reads `if config.airport_admit_on_ground and "on_ground" in sv_f.columns`. A feature gate of that shape does not raise on a missing column — it silently reverts to the altitude-only condition. So the setting was on, the gate was correct, and the measurement that resulted was read as evidence about radio reception for two full campaigns. A stand sighting that was never admitted and one that was never received are the same absent row. Fixing the projection took pooled AOBT coverage from 9.10% to 90.13% and AIBT from 7.85% to 87.59%. **Feature-gating on column presence should be treated as a defect, not a defensive idiom:** it converts a schema regression into a silent behaviour change. |
| 31 | **That fix is unconditional, so no rung measures it.** | It is a correction to which columns step 04 reads, not a behaviour worth a flag — every rung carries it, and V00 reports the same block coverage as V07. The before/after is against the *previous run*, staged from this repo's own git history into `papers/flight-events-v4/data/blocks_before_2026.csv` by `tools/stage_blocks_before.py`, with the commit that produced it in every row. Staging it rather than hardcoding it was not ceremony: the prose had been claiming a pooled before-coverage of 7.30%/6.58% and an EDDS before-value of 1.63% while the committed data for the run it named said 9.10%/7.85% and 42.51%. The literals had drifted a whole run behind the text around them, and nothing could detect it because nothing read them from anywhere. `tools/check_literals.py` now fails the render on any measurement in prose that does not read a staged CSV or carry `<!-- literal-ok: reason -->`. |
| 32 | **A job that does not declare an output does not get one, and staleness cannot see it.** | `events_compare.py` writes seven CSVs; the compare job declared three. `regenerate` copies only declared outputs into the paper's `data/` and computes staleness only over declared outputs — so four were written to the staging directory and dropped, the paper kept whatever copies were already there, and `--check` reported the job `ok`. The paper read three of them, and their manifest entries showed a manual invocation at `39093a1` with `--driver-memory 16g`: three commits before the `on_ground` fix. The manifest had recorded this faithfully the whole time; nothing was reading it. `floor_{period}.csv` is deliberately *not* declared, because the inter-source floor needs `apdf_full_202606`, which has never been extracted — it comes back empty and no file is written. |
| 33 | **The `opensky` driver heap (10 GB) exceeds what this pod can spare, and the ladder run died of it.** | The cgroup cap is 16 GB and is shared; long-lived agent sessions routinely hold 8 GB. A JVM entitled to more than is free does not fail politely — it grows until the kernel SIGKILLs it, with no Java OOM and no crash dump. An eight-rung ladder keeps one SparkContext across every rung and retains UI state per stage; this run reached stage 2102 in one context and the driver crept to 7.3 GB over fifteen hours before dying on the final rung. Retained state is now capped, `--driver-memory` reaches the distributed builder (it was read only by the local-mode branch, so every distributed run had silently used 10 GB whatever the caller asked), and the default is 6 GB. Rung scores accumulate in driver memory until the last rung, so that crash lost all eight sets of numbers while leaving seven completed tables intact: `--reuse-table`, gated by a mandatory `--reuse-newer-than`, scores a rung from its existing table. The gate is not paranoia — `research/events_2026_V07_shipped` held a pre-fix table from an earlier run at the moment the ladder died. |
| 34 | **Two events for one question is a decision the consumer should not have to make.** | `events_v0.2.0` published `ATOT` beside `airborne` and `ALDT` beside `touchdown` — decision 29's reading of the measurement, and correct as far as it went. What it left behind was a table where answering "when did this take off" required knowing which of two rows to trust, and the right answer varies by aerodrome: A-CDM is the better instant where surface reception reaches (the interpolated 15 ft crossing against legacy's extreme-of-window, +19 s median bias) and absent where it does not (4–7% of the network against ~90%). `events_v0.3.0` merges each pair at source, A-CDM preferred, legacy filling the rest. Being a coalesce, coverage can only rise. `info.method` carries the arm **on the event, not in the flight list**, because the merged column is a mixture of two estimators whose mixing ratio varies by aerodrome — an aggregate over it carries an aerodrome-dependent bias that is invisible in the column itself, and the only defence is that the mixture stays recoverable. |
| 35 | **`level-start` outnumbered `level-end` by 25,369 a day, and the cause was a `when` chain, not the data.** | Measured on 2026-06-01: 129,426 starts against 104,057 ends, 28,855 unmatched across 17,356 of 44,461 flights. Not a boundary artefact — `lag`/`lead` default to the *string* `"None"`, so a track beginning or ending level is labelled correctly at both ends. The chain in `calculate_horizontal_segment_events` was first-match-wins with four things to say in one slot, so a sample that was both the start and the end of a segment matched the "level-start" branch and could never reach the "level-end" one. Only 22% of the unmatched starts fell within ten minutes of `last_seen`; the rest were mid-flight, which is where one-sample segments live. Dropping the fuzzy tops (decision 36) frees two slots and what remains is two independent predicates, so the defect is removed structurally rather than special-cased. Reproduced in a unit test: a single cruise sample gives `level-start 1, level-end 0, top-of-climb 1, top-of-descent 0` under v0.2.0 and a balanced pair under v0.3.0 — note the unreachable branch cost the `level-end` **and** its `top-of-descent` together. |
| 36 | **The tops keep one method, and it is the PRU one.** | No source available to OPDI records a top of climb, which is why decision 25 published both arms rather than choosing between them. That reasoning justified publishing both under *different names*; it never justified leaving the choice open forever. The PRU arm is the better-defined (a stated methodology with an exclusion box, against a fuzzy classifier whose cruise rule centres on 600 kt and so gives turboprops none) and the better-covered — 55,819 against 32,604 a day for the climb, 55,810 against 32,541 for the descent. It therefore takes the plain `top-of-climb`/`top-of-descent` names and the suffixed `-cco`/`-cdo` pair ceases to exist. |
| 37 | **A test that drives a detector through a column the detector does not read measures nothing.** | The first version of the decision-35 tests set `flight_phase` on the frame. `calculate_horizontal_segment_events` selects only `track_id`, position, `baro_altitude_c`, `vert_rate`, `velocity` and the cumulative measures, and recomputes the phase itself — so the labels were ignored and three of four tests passed for reasons unrelated to what they claimed. The replacement drives the phase through the physics, and needs a 60 s sample step: at 5 s a single cruise sample is erased by `phase_twindow_seconds` smoothing as a flicker and the trajectory never reaches cruise at all, so the first attempt at *that* measured nothing either. **A test against a detector that derives its own inputs must assert the derived value, not the input it was handed.** |
| 38 | **An enrichment that writes back to the table it read must not write only the window it read.** | Step 04b adds twenty-two columns to `opdi_flight_list`. Scoped to the campaign window it would have left every other partition — including the `DOF=__HIVE_DEFAULT_PARTITION__` one holding rows with no date — on the pre-enrichment schema. Spark does not merge parquet schemas by default; it takes the schema of one file it happens to sample. A table whose partitions disagree therefore shows or hides the new columns depending on which file that is, and the failure is silent: the reader opens the flight list and sees no milestones on it. 04b rewrites every partition, which is affordable only because this table is ~3 MB a day, and is self-healing for a day whose events landed after its first enrichment. Separately, the write goes through a staging table: the enriched frame derives from the table it replaces, and Spark's lazy write would otherwise delete partitions the plan still had to read, succeeding with whatever it had scanned first. |
| 39 | **The level imbalance had two causes, and the first fix only found one.** | Decision 35 removed the first-match-wins precedence and predicted balance. Measured on the campaign's first day it went from +25,369 to **+15,682** -- better, not fixed, and reporting it as fixed would have been wrong. The remainder is a NULL-safety bug: the classifier abstains (returns NULL) when a rule's inputs are incomplete, and `NULL != 'LVL'` is NULL rather than TRUE, so a level segment adjacent to an abstention failed both the start test and the end test. It is not symmetric, which is why it hid behind the first fix: a *cruise* segment is rescued by the `first_cr_time`/`last_cr_time` terms, which fire regardless of the neighbour, while a level-off below cruise has no rescue and so emitted a start at its clean edge and no end at its abstaining one -- a systematic surplus of starts, matching the sign of the residual. Fixed with `eqNullSafe`, which is also the right reading: "we do not know what this sample is" is not a continuation of level flight. **The first hypothesis tested clean because the probe used cruise**, where the rescue masks the defect; the test that catches it sits at 15,000 ft and 300 kt, level but not cruise, so the track holds no `CR` sample and neither rescue term exists. A probe built from the common case cannot see a bug the common case is immune to. |
| 40 | **A NULL partition value is `__HIVE_DEFAULT_PARTITION__`, and formatting it as `None` duplicates the table.** | `storage._delete_partition_paths` interpolated the partition value straight into the path, so a NULL produced `DOF=None` -- a directory that does not exist. The delete found nothing, succeeded silently, and the append that followed landed *beside* the rows it was meant to replace rather than instead of them. `opdi_flight_list` holds 746 rows with no date; after a single re-enrichment the table carried two copies of each, three pre-enrichment files and one new one all live. Step 04b rewrites every partition on every day, so the fortnight would have ended with fifteen copies. This affects any partitioned overwrite whose frame contains a NULL partition value, not only 04b, and it was found by pulling the table to CSV rather than by any test -- the failure leaves a *successful* write behind it. |
| 41 | **A quarter of the flight list is not flights, and every coverage figure is diluted by it.** | 13,429 of 52,400 rows on 2026-06-01 carry no milestone and no ring crossing. 6,148 have no aerodrome at either end, so nothing can anchor; of the 2,668 that have both, **1,914 have `ADEP == ADES`** -- EGLL->EGLL, LFBZ->LFBZ, EDDB->EDDB -- circuits, towing and ground movement. The discriminator is duration: rows *with* data run a median of 86 minutes and only 0.4% are under five, while the empty ones run 17-18 minutes. This resolves two anomalies reported earlier as detector problems. EBBR shows 534 departure legs against the ~230 the aerodrome operates, and `ATOT` reaching only 55.4% of them; filter to legs whose track ran over 30 minutes **and** names a destination and it is **225 legs, 222 with an `ATOT` -- 98.7%**. The detectors were not the problem and the aerodrome assignment was not wrong; step 03 admits five-minute fragments as flights and nothing downstream distinguishes them. **Any coverage statistic computed over the raw flight list understates the detectors by roughly a quarter.** Whether the published table should carry a duration column, a fragment flag, or a filter is a product decision and is open. |
| 42 | **The callsign-flicker hypothesis (track-splitting debounce, Task 1) is CONFIRMED -- but the first measurement used the wrong denominator, and it took a second pass (Task 1b) to see that.** | First pass: `benchmarks/callsign_flicker.py` measured the raw per-sample callsign sequence on `osn_tracks_clean` for 2026-06-01 (icao24-partitioned, blanks ignored). Of 36,012 raw non-blank callsign changes, 5,241 (14.6%) are `X -> Y -> X` flickers, held briefly (p50/p90/p99 = 5s, max 21s) -- read against the brief's literal `>30%`-of-raw-changes gate, that read as REFUTED. It was the wrong comparison: most raw callsign changes are legitimate leg-to-leg changes that *should* split a track, so comparing flickers against that population says nothing about whether flicker causes spurious splits. Task 1b instead ran the shipped rule itself (`assign_track_id(sv, recommended(), SegmentationParams.from_config(...))`) and classified every track boundary it actually drew on 2026-06-01, in precedence order (gap > 30 min; gap > 15 min & alt < 5,000 ft; the boundary is a flicker's revert leg; a genuine non-reverting callsign change; else unexplained). Of **44,406 boundaries**: 30,961 (69.7%) gap>30min, 5,241 (11.8%) flicker (the `Y -> X` revert leg), 5,000 (11.3%) unexplained, 2,199 (5.0%) genuine callsign change, 1,005 (2.3%) gap+low-alt. The "unexplained" bucket is not a mystery residual -- by construction (a non-gap boundary can only be `recommended`'s callsign_change term) every one of those 5,000 rows is the *other* leg of a detected flicker, the `X -> Y` excursion, which the classification's literal "is the sample the X of an X->Y->X" wording excluded from the `flicker` label because at that boundary the callsign is Y, not X. Counting both legs of one flicker event as one cause (which is the right count -- a single flicker produces two of `recommended`'s track boundaries, not one) gives **10,241 of 44,406 boundaries, 23.1%**, attributable to flicker. Stated precisely rather than as a residual (code review, fix round 1, finding 2 -- the `.when()`/`.otherwise()` classification is exhaustive by construction, so a literal "zero residual" is tautological, not a finding): of the **12,440 callsign-change boundaries** (2,199 + 5,241 + 5,000), 2,199 match no detected flicker leg and **10,241 match at least one**. That is thousands, non-negligible, and clears the bar the brief's Step 4 intended even though the specific `>30% of raw changes` threshold it wrote down did not test the right thing. **Caveat added in fix round 1, finding 1:** the flicker bucket's exact 5,241-count match against the first pass is not, by itself, proof of a clean one-to-one correspondence -- **240** of the 44,406 boundaries match *both* a flicker's revert leg and a neighbouring flicker's excursion leg (back-to-back flickers on one airframe, X,Y,X,Y,X with no stable sample between them; LBT808 and MSR730 in the sample output show exactly this pattern), and `.when()` precedence resolves those silently to `flicker`. The 23.1% headline is unaffected (it sums both buckets regardless of which one a collision lands in), but the exactness of the count match is partly coincidental rather than fully structural. **Verdict: CONFIRMED.** Tasks 2-6 proceed. |
| 43 | **A flicker trips the boundary rule twice, and it trisects a flight rather than bisecting it -- the persistence threshold follows from the flicker's measured duration, not from a guess.** | Restated precisely for the record this decision feeds: of the 44,406 track boundaries `recommended` draws on 2026-06-01, 23.1% (10,241) trace to a callsign flicker leg (decision 42). A flicker is `X -> Y -> X`; it trips the callsign-change boundary rule on both the `X -> Y` excursion and the `Y -> X` revert, so one flicker event produces two of the rule's boundaries and turns one flight into three tracks, not two. Flicker holding time measured p50 = p90 = p99 = 5 s (one sample), max 21 s -- essentially all single-sample events. The chosen debounce threshold, `callsign_min_persistence_seconds = 30`, is set from that distribution: comfortably above the measured max (21 s, ~1.4x) and well past p99, so a callsign must hold for 30 s of continuous observation before a change is treated as real, which covers every flicker measured on the sample day with margin rather than sitting at the median where an unlucky longer flicker would still slip through. |
| 44 | **A9 `debounced` suppresses callsign-flicker splits without merging flights, and it defaults off.** | New segmentation arm A9 (`debounced`, `src/opdi/pipeline/segmentation/`), selected via `SegmentationConfig.method` plus a new `callsign_min_persistence_seconds` field threaded through `SegmentationParams`; at `= 0` it is textually identical to A8 `recommended`. Run-level persistence verdict (not a per-row check -- Task 3's first per-row attempt over-suppressed multi-sample flickers and was corrected): a callsign counts as held only if the whole run of samples on one side of a transition persists for the threshold, comparing persisted-to-persisted rather than persisted-to-previous-raw. Measured on 2026-06-01 at `hold = 30 s`: 46,676 tracks against recommended's 57,689 -- a ~19% fragmentation drop, consistent with removing ~5,100 flickers at ~2 tracks each (revised from an earlier ~2-3% prediction that pre-dated the Task 1b trisection finding). The metric that distinguishes flicker suppression from genuine over-merging is tracks holding 2+ **persisted** callsigns (each spanning >= the hold threshold), not the raw `collect_set` count, which rises to 3,056 by design because a debounced track deliberately keeps the flicker's garble alongside the real callsign. The persisted-callsign guard stays near recommended's 47 across the tested range -- 289 at hold=15s, 259 at hold=30s, 223 at hold=60s -- shrinking as hold rises, the opposite of what a genuine two-flight merge would show (a real merge is hold-invariant, since both callsigns already span minutes). A density-refined genuine-merge comparison (span >= 300 s & samples >= 100 & dissimilar callsigns), run on both arms to isolate the arm's own effect: recommended 36 fusions/day, debounced 37 -- 1.03x, and the same RYR/EJU consecutive-rotation pairs appear in **both** arms, so the ~37 are pre-existing in the shared gap/recent-lookback rules and not introduced by debouncing. Verdict: SOUND. `callsign_min_persistence_seconds` defaults to `0`, so A9 ships available but inert; promoting it changes `track_id` shape for every future dataset, the same cost A8 itself carried on 2026-08-27, and is a decision for the owner of the published contract, not taken here. |
| 45 | **The APDF movement-count guard passed, with a caveat it is not a clean arm-delta -- and APDF block-time timing was deliberately never used for any of this work.** | On 2026-06-03 (a full-pipeline run into a redirected warehouse, current-branch code), debounced gave departure/arrival movement counts of 1.01x/1.01x against APDF, zero real movements wrongly excluded, and a tighter per-aerodrome spread than the recommended baseline (opdi-prod, built by the events_v0.3.0 campaign) at 1.14x/0.99x. Caveat: the opdi-prod baseline predates the ghost rule, the aerodrome correction and the `MOVEMENT_DEP`/`MOVEMENT_ARR` columns, so the apparent 1.14x -> 1.01x departure improvement conflates the arm change with a code-vintage change and must not be attributed to debouncing; a clean arm-only comparison would need rebuilding recommended with current code, not done because the guard's only question -- does debouncing break movement counting -- is already answered. Why `AOBT`/`AIBT` timing was not used, here or anywhere in this investigation: APDF block times are frequently rounded to the whole minute, so a timing delta of tens of seconds would measure APDF's own quantisation rather than the detector; and ADS-B presence does not share an end with the block times, because a transponder can stay on long after on-block or switch on well before off-block. Both are properties of the reference data, not defects in it, but together they mean a timing comparison here would produce a number with no interpretation. Only movement counts and (where used elsewhere) categorical runway identity, both immune to rounding and transponder dwell, serve as guards -- never as the measured result. |

...
| 42 | **The callsign-flicker hypothesis (track-splitting debounce, Task 1) is REFUTED: X→Y→X reverts are 14.6% of raw callsign changes, not the majority the gate needed.** | `benchmarks/callsign_flicker.py` measured the raw per-sample callsign sequence on `osn_tracks_clean` for 2026-06-01 (icao24-partitioned, blanks ignored). Of 36,012 raw non-blank callsign changes, 5,241 (**14.6%**) are `X -> Y -> X` flickers -- well under the `task-1-brief.md` gate's `>30%` threshold for confirming the hypothesis. Where a flicker does occur it is brief (held-time p50/p90/p99 = **5s**, max **21s**), so the *duration* half of the hypothesis holds, but the *share* half does not, and the gate requires both. Sampled flickers are mostly single/double-character ADS-B decode corruption riding on a genuine callsign (`MSR729 -> BDP6 -> MSR729`, `MSR730 -> CJ614 PW -> MSR730`, `LBT808 -> BHP7N P -> LBT808`), not a callsign transmitting, reverting, and holding. Per the brief's Step 4: **Tasks 2-6 do not proceed off this run.** They are built on "sub-minute splits are mostly callsign flicker," and 85.4% of raw callsign changes -- the majority driving sub-minute splits -- are something else, not yet diagnosed. Whatever is producing the bulk of the 1,765 sub-minute splits and their 1,457 same-`FLT_ID` halves (the number that motivated this task) needs a fresh hypothesis before a debounce threshold is designed against it. |

...
df = spark.table("project_aiu.osn_tracks_clustered")

# Update the track_id column by appending the year and month from event_time
...
# Drop the old track_id column and rename the new_track_id column to track_id
final_df = updated_df.drop("track_id").withColumnRenamed("new_track_id", "track_id")

spark.sql("""DROP TABLE IF EXISTS project_aiu.osn_tracks_clustered_updated;""")

# Create an empty table with desired schema and clustering properties
spark.sql("""
CREATE TABLE project_aiu.osn_tracks_clustered_updated (
...
CLUSTERED BY (event_time, icao24, callsign, track_id) INTO 4096 BUCKETS
STORED AS parquet
TBLPROPERTIES ('transactional'='false');
""")

# Populate the new table with data from the DataFrame
final_df.write.insertInto("project_aiu.osn_tracks_clustered_updated")
...
while h3_resolution >= 0:
    h3_res_sql = h3_res_sql + f"h3_id_res_{h3_resolution} STRING COMMENT 'H3 cell identifier for lat and lon with H3 resolution {h3_resolution}.',"
    h3_res.append(f"h3_id_res_{h3_resolution}")
...
original_columns = df.columns + ['track_id'] + h3_res
...
for idx, month_id in enumerate(unique_month_ids):
    ...
    while h3_resolution >= 0:
        if h3_resolution == max_h3_resolution:
            ...
            df_month = df_month.withColumn("h3_resolution", lit(h3_resolution))
            df_month = df_month.withColumn(f"h3_id_res_{h3_resolution}", h3_pyspark.h3_to_parent(F.col(f"h3_id_res_{max_h3_resolution}"), F.lit(h3_resolution)))

...
def load_airports_hex(spark, resolution = 7):
    ...
    try:
        ...
        df_apt = pd.read_parquet(f'data/airport_hex/airport_concentric_c_hex_res_{resolution}_processed_v2.parquet')
        ...
        df_apt = pd.read_parquet(f'data/airport_hex/airport_concentric_c_hex_res_{resolution}_v2.parquet')
        df_apt = df_apt[df_apt.type.isin(['large_airport', 'medium_airport', 'small_airport'])]
        ...
        df_apt.to_parquet(f'data/airport_hex/airport_concentric_c_hex_res_{resolution}_processed_v2.parquet')
...
def process_flight_table_DAI(spark, project, month):
    ...
    flight_table.writeTo(f"`{project}`.`opdi_flight_list_v2`").append()
...
def fetch_and_aggregate_sv_overflight(spark, project, month):
    ...
    fl = get_data_within_timeframe(
        spark=spark,
        table_name=f'{project}.opdi_flight_list_v2',
        month=month,
        time_col='first_seen'
    ).select('id')
